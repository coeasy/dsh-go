...
import {
  ...
  distributionPackageKey,
  writeRegistryDistribution,
} from '../../scripts/registry-distribution.mjs';
...
function packageRecord(id: string, type = 'plugin', commitSeed = 'a') {
  ...
  return {
    ...
    source: {
      ...
      ref: 'main',
      commit,
      archive_url: `https://github.com/fixture/${id}/archive/${commit}.tar.gz`,
    },
...
function registry(records: any[], at = '2026-08-26T00:00:00.000Z') {
  const value: any = {
    ...
    generated: { at, count: records.length, content_hash: '' },
    plugins: records,
  };
...
describe('Registry Distribution V1', () => {
  it('builds 256 deterministic shards, package records and a package-level delta', async () => {
    ...
    expect(index.distribution_version).toBe(1);
    expect(index.registry_version).toBe(3);
    ...
    expect(index.delta.changed).toBe(2);
    expect(index.delta.removed).toBe(1);

    const alphaKey = distributionPackageKey('plugin', 'alpha');
    const alphaDescriptor = index.packages[alphaKey];
    expect(alphaDescriptor.path).toMatch(/^packages\/[0-9a-f]{2}\/[0-9a-f]{64}\.json$/);
    const alpha = JSON.parse(await readFile(resolve(out, alphaDescriptor.path), 'utf8'));
    expect(alpha.key).toBe(alphaKey);
    expect(alpha.entries[0].package.source.commit).toBe('c'.repeat(40));
    ...
    expect(direct.record.entries[0].package.id).toBe('alpha');
  });

  it('uses one conditional index request and zero shard downloads when the distribution hash is unchanged', async () => {
    const root = await mkdtemp(join(tmpdir(), 'dsh-registry-distribution-http-'));
    const out = join(root, 'public', 'distribution-v1');
    const current = registry([
      packageRecord('alpha', 'plugin', 'a'),
      packageRecord('beta', 'plugin', 'b'),
      packageRecord('gamma', 'mcp', 'c'),
    ]);
      ...
      try {
        ...
        if (path === 'distribution-v1/index.json') {
          ...
          if (req.headers['if-none-match'] === index.etag) {
            res.writeHead(304, { ETag: index.etag });
            res.end();
            return;
          }
          res.writeHead(200, { 'Content-Type': 'application/json', ETag: index.etag });
          res.end(await readFile(join(out, 'index.json')));
          return;
        }
    ...
    expect(JSON.parse(await readFile(cacheFile, 'utf8')).plugins).toHaveLength(3);
  });
  ...
  it('falls back to the legacy full Registry V3 when distribution retrieval fails', async () => {
    ...
    const server = createServer((req, res) => {
      if (req.url === '/distribution-v1/index.json') {
        res.writeHead(503, { 'Content-Type': 'application/json' });
        res.end(JSON.stringify({ error: 'distribution unavailable' }));
        return;
      }
      if (req.url === '/registry-v3.json') {
        res.writeHead(200, { 'Content-Type': 'application/json', ETag: '"legacy-v3"' });
