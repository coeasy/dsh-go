...
export const onRequestGet: PagesFunction<Env> = async ({ request, env, params }) => {
  ...
  try {
    ...
    const id = `${param(params.owner)}/${param(params.name)}`;
    const pkg = findRegistryPackage(data, param(params.type), id);
    if (!pkg) throw new ProtocolError(ERROR_CODES.PACKAGE_NOT_FOUND, 'package not found');
    return apiData({ package: `${pkg.type}:${pkg.id}`, releases: sortedReleases(pkg) }, { ...meta, registry_revision: data.revision });
  } catch (error) {
