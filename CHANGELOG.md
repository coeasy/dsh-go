...
- **README 最近热门推荐表**：自动生成 DSH 原生插件（仅收录命名含 `dsh` / `deepseek-harness`，剔除仅打 `dsh-plugin` 标签但无关的老开源项目）的最近更新 Top20 表格，每次同步后由 CI 重写，始终展示最新；区间放宽至 **300-5000★** 扩大覆盖，新增 **pinned 固定推荐**（modlens、DSH-better-sidebar 强制置顶入选），并为生成器补充单元测试（vitest 已支持 `*.test.mjs`）
